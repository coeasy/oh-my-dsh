...
- That directory is gitignored. Fetch it with `pnpm fetch:engine` or `tools\build-clients.cmd`.
...
pnpm compile
pnpm verify
tools\build-clients.cmd          # Windows: VSIX + NSIS/portable/zip
./tools/build-clients.sh          # macOS/Linux: VSIX + dmg or AppImage + zip
