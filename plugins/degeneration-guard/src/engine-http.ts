/**
 * Engine-side HTTP API for the Degeneration Guard plugin.
 *
 ...
 *
 * Routes:
 *   GET /degeneration-guard/api/call?method=<name>&args=<json array>
 *
...
export type DegenerationGuardServiceLike = Record<string, unknown>
...
export function registerDegenerationGuardHttpApi(
  webServer: WebServerLike,
  getService: () => unknown,
): void {
  webServer.register({
    kind: 'exact',
    path: '/degeneration-guard/api/call',
    handler: (req, res) => {
      ...
      try {
        ...
        if (typeof fn !== 'function') {
          sendJson(res, 400, { ok: false, error: `unknown degeneration-guard method: ${method}` })
          return
        }
