/**
 * Plugin manifest validation (install/load-time) for the degeneration guard.
 *
 * Permissions are intentionally narrower than model-config: runtime
 * observation + interruption only, no settings persistence beyond its own
 * local config.
 */

export const PLUGIN_ID = 'com.my-dsh.degeneration-guard'
export const PLUGIN_NAME = 'Degeneration Guard'
...
/** Permissions this plugin is allowed to request. Anything else is rejected. */
...
/** Permissions the plugin explicitly forbids. */
...
export function defaultManifest(): PluginManifest {
  return {
    ...
    permissions: ['session.observe', 'agent.interrupt', 'events.subscribe', 'ui.mount'],
