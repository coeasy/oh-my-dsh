...
/** SVG entry. `qrcode` Node main pulls pngjs `require('fs')`, which the ESM Electron bundle rejects. */
import QRCode from 'qrcode/lib/browser.js'
import {
  renderDesktopPairingPage,
  renderMobilePage,
  renderPairingWaitPage,
} from './lan-mobile-pages.ts'
...
export class LanMobileBridge {
  ...
  private readonly options: LanMobileBridgeOptions

  constructor(options: LanMobileBridgeOptions) {
  ...
  private async handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    if (request.method === 'GET' && url.pathname === '/desktop') {
      ...
      if (!snapshot.pairingUrl || !snapshot.expiresAt)
        return this.text(response, 503, 'Bridge unavailable.')
      const qrSvg = await QRCode.toString(snapshot.pairingUrl, {
        type: 'svg',
        margin: 1,
        width: 260,
      })
    ...
    if (request.method === 'GET' && url.pathname === '/desktop/pending') {
      ...
      return this.json(
        response,
        200,
        pending ? { id: pending.id, remoteAddress: pending.remoteAddress } : {},
      )
    }
    ...
    if (request.method === 'GET' && url.pathname === '/pair/status') {
      ...
      response.setHeader(
        'set-cookie',
        `dsh_mobile=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=31536000`,
      )
    ...
    if (request.method === 'POST' && url.pathname === '/api/rpc') {
      ...
      if (typeof input.method !== 'string' || !RPC_ALLOWLIST.has(input.method)) {
        return this.json(response, 403, {
          ok: false,
          error: 'RPC method is not available on mobile.',
        })
      }
  ...
  private async forwardRpc(
    ...
    if (!response.ok)
      return { ok: false, error: `Harness transport returned HTTP ${response.status}.` }
