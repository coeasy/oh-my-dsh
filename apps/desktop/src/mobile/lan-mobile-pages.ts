...
<body><main class="shell">
  ...
  <section id="sessionsView" class="view active">
    <div class="session-actions"><select id="workspace" aria-label="${zh ? '当前工作区' : 'Current workspace'}"></select><button id="newSession" class="new-session" disabled>⊕ ${zh ? '新会话' : 'New Session'}</button><button id="refresh" class="icon-button" aria-label="${zh ? '刷新' : 'Refresh'}"><svg width="17" height="17" viewBox="0 0 20 20" fill="none"><path d="M16 7.5A6.5 6.5 0 1 0 16 13M16 7.5V3.5M16 7.5H12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></button></div>
    <div id="workspaceHint" class="workspace-hint"></div>
...
const L=${JSON.stringify({
    ...
    output: zh ? '输出' : 'Output',
    noWorkspaces: zh
      ? '暂无工作区，请先在 DSH Desktop 中创建第一个工作区。'
      : 'No workspaces yet. Create your first workspace in DSH Desktop.',
...
function renderBlock(block,streaming){if(!block)return'';if(block.kind==='text')return'<div class="markdown'+(streaming?' streaming':'')+'">'+markdown(block.text)+'</div>';if(block.kind==='reasoning')return'<details class="thinking"'+(streaming?' open':'')+'><summary><span>Think'+(streaming?' · '+L.running:'')+'</span><span>›</span></summary><div class="thinking-body markdown'+(streaming?' streaming':'')+'">'+markdown(block.text)+'</div></details>';if(block.kind==='tool'){const label=block.status==='running'?L.running:block.status==='error'?L.toolFailed:L.done;return'<details class="tool"><summary><span class="tool-name">'+esc(block.name||'tool')+'</span><span class="tool-status '+esc(block.status)+'">'+label+'</span></summary><div class="tool-body">'+(block.args?'<strong>'+L.input+'</strong><pre>'+esc(pretty(block.args))+'</pre>':'')+(block.result?'<strong>'+L.output+'</strong><pre>'+esc(block.result)+'</pre>':'')+'</div></details>'}return''}
...
async function loadWorkspaces(){const previous=$('workspace').value,value=await rpc('workspace.list');workspaces=value.items||[];if(workspaces.length){$('workspace').innerHTML=workspaces.map(w=>'<option value="'+esc(w.workspaceId)+'">'+esc(w.title||w.path)+'</option>').join('');$('workspace').value=workspaces.some(w=>w.workspaceId===previous)?previous:workspaces[0].workspaceId}else $('workspace').innerHTML='<option value="">'+L.noWorkspaces+'</option>'}
...
async function loadSessions(){try{const value=await rpc('session.list',{}),wid=$('workspace').value,w=workspaces.find(x=>x.workspaceId===wid),allowed=w?new Set(w.sessionIds):new Set(),items=(value.items||[]).filter(s=>allowed.has(s.sessionId)).sort((a,b)=>b.updatedAt-a.updatedAt);$('sessions').innerHTML=!wid?'':items.length?items.map(s=>{const title=titleFor(s);return'<button class="row" data-id="'+esc(s.sessionId)+'" data-title="'+esc(title)+'"><strong>'+esc(title)+'</strong><time>'+esc(relativeTime(s.updatedAt))+'</time></button>'}).join(''):'<div class="empty">'+L.noSessions+'</div>';document.querySelectorAll('.row').forEach(b=>b.onclick=()=>openSession(b.dataset.id,b.dataset.title))}catch(e){$('listError').textContent=e.message}}
...
export function renderDesktopPairingPage(options: {
  ...
  const text = {
    ...
    heading: zh ? '连接你的手机' : 'Connect your phone',
    hint: zh
      ? '请确保两台设备连接到同一个可信 Wi-Fi，然后扫描二维码。'
      : 'Keep both devices on the same trusted Wi-Fi, then scan the QR code.',
    manageHeading: zh ? '管理手机连接' : 'Manage phone connection',
    manageHint: zh
      ? '这台手机当前已连接到 DSH Desktop。'
      : 'Your phone is currently connected to DSH Desktop.',
    connected: zh ? '手机已连接' : 'Phone connected',
    closeHint: zh
      ? '连接会在后台保持，现在可以关闭此窗口。'
      : 'The connection stays active in the background. You can close this window now.',
    done: zh ? '完成' : 'Done',
    ...
    expired: zh
      ? '二维码已过期，请重新打开此窗口刷新。'
      : 'QR expired. Reopen this window to refresh.',
  }
